...
while($sueros = mysqli_fetch_assoc($res_sueros)){
    ...
    if($array[1] == "on"){
        ...
        if($valida == 0){
            //print_r($array);
            $sql_sv = "INSERT INTO rec_sueros(suero, comp1, comp2, comp3, comp4, comp5, comp6, comp7, id_cita, user_registra)
                        VALUES ('$array[0]','$array[2]', '$array[3]','$array[4]', '$array[5]', '$array[6]', '$array[7]', '$array[8]','$id_cita','$user')";
                if($mysqli -> query($sql_sv) === true){
                    echo '<p>El suero: '.$array[9].', fue registrado correctamente con sus complementos</p>';
                }else{
            ...
            $suero_prev = mysqli_fetch_assoc($val);
                    $sql_up = "UPDATE rec_sueros SET comp1 = '$array[2]', comp2 = '$array[3]', comp3 = '$array[4]',
                                                    comp4 = '$array[5]', comp5 = '$array[6]', comp6 = '$array[7]', comp7 = '$array[8]', user_registra = '$user'
                               WHERE suero = '$array[0]' and id_cita = '$id_cita'";
                    if($mysqli -> query($sql_up) === true){
                        echo '<p>El suero: '.$array[9].', ya había sido registrada previamente, se han actualizado los complementos</p>';
                    }else{
