...
<div class="container" style="margin-bottom: 5%;">
    <div class="row center-align">
        <div class="col s12">
            <h4 style="color: #2d83a0; font-weight:bold;">Buscar Paciente para Capturar Historia Clínica</h4>
            <br>
...
<script>
    $(document).ready(function(){
        ...
        $('#buscar_paciente').on('keyup', function(){
            ...
            searchTimeout = setTimeout(function(){
                $.ajax({
                    type: 'POST',
                    url: 'logic_consulta/busca_pacientes_ajax.php',
                        ...
                        if(response.success){
                            if(response.resultados.length > 0){
                                ...
                                $.each(response.resultados, function(index, paciente){
                                    ...
                                    html += '<button class="btn-accion btn-captura" onclick="irACaptura(' + paciente.id_paciente + ')"><i class="material-icons" style="font-size: 14px; vertical-align: middle;">edit</i> Capturar</button>';
                                    html += '<button class="btn-accion btn-imprimir" onclick="irAImprimir(' + paciente.id_paciente + ')"><i class="material-icons" style="font-size: 14px; vertical-align: middle;">print</i> Imprimir</button>';
                                    html += '</div>';
                                    html += '</div>';
                                });
    ...
    // Función para ir a la captura de historia clínica del paciente
    function irACaptura(idPaciente){
        window.open('captura_hcg.php?id_paciente=' + idPaciente + '&flag_hcg=1', '_blank');
    }

    // Función para ir a imprimir historia clínica del paciente
    function irAImprimir(idPaciente){
        window.open('print_h_clinica.php?id_paciente=' + idPaciente, '_blank');
    }
</script>
</body>
</html>